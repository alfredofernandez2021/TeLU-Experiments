...
class Sish_Functional(torch.autograd.Function):
    @staticmethod
    def forward(ctx, input):
        ctx.save_for_backward(input)
        #exp = torch.sinh(input) + torch.cosh(input)
        #exp = input
        return input * torch.tanh(torch.exp(input))
    ...
    def backward(ctx, grad_output):
        ...
        #exp = torch.sinh(input) + torch.cosh(input)
        #exp = input
        sech2 = (1/(torch.cosh(torch.where(input>10, 22026.466*input-198238.192, torch.exp(input)))))**2
        #sech2 = torch.where(input<5, 1, torch.where(input>5, 0, (1/(torch.cosh(torch.exp(input))))**2))
        return torch.tanh(torch.exp(input)) + torch.where(input>10, 22026.466*input-198238.192, torch.exp(input))*input*sech2
        #return (1 / (1+torch.exp(input))) * (1-(1 / (1+torch.exp(input)))) 
...
def ResNet18_Smish():
    return ResNet_Smish(BasicBlock_Smish, [3, 4, 6, 3])
...
def ResNet18_Logish():
    return ResNet_Logish(BasicBlock_Logish, [3, 4, 6, 3])
...
def ResNet18_Mish():
    return ResNet_Mish(BasicBlock_Mish, [3, 4, 6, 3])
...
def ResNet18_SiLU():
    return ResNet_SiLU(BasicBlock_SiLU, [3, 4, 6, 3])
...
def lr_schedule(lr, epoch):
    ...
    return lr * math.pow(0.3, optim_factor)
...
def train(epoch, activation):
    ...
    correct    = 0
    total      = 0
    optimizer  = optim.SGD(net.parameters(), lr=lr_schedule(lr, epoch), momentum=0.0, weight_decay=3e-3)#was1e-3
...
with open('ResNet_CIFAR100_Trial2.csv', 'w') as myfile:

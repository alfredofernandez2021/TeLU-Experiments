...
def ResNet18_Sish():
    return ResNet_Sish(BasicBlock_Sish, [3, 4, 6, 3])
...
def ResNet18_GELU():
    return ResNet_GELU(BasicBlock_GELU, [3, 4, 6, 3])
...
def ResNet18_ReLU():
    return ResNet_ReLU(BasicBlock_ReLU, [3, 4, 6, 3])
...
def train(epoch, activation):
    ...
    correct    = 0
    total      = 0
    optimizer  = optim.SGD(net.parameters(), lr=lr_schedule(lr, epoch), momentum=0.0, weight_decay=3e-3)#achieves 72% with 4e-3

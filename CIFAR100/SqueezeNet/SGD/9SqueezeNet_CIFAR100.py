...
train_dataset   = torchvision.datasets.CIFAR100(root='./data', transform=transform_train, train=True, download=True)
train_dataset = torch.utils.data.Subset(train_dataset, range(45000))
print(len(train_dataset))
valid_dataset   = torchvision.datasets.CIFAR100(root='./data', transform=transform_valid, train=True, download=True)
valid_dataset = torch.utils.data.Subset(valid_dataset, range(45000,50000))
print(len(valid_dataset))
test_dataset    = torchvision.datasets.CIFAR100(root='./data', transform=transform_test, train=False, download=True)
...
def SqNxt_23_1x_Sish(num_classes):
    return SqueezeNext_Sish(2.0, [6, 6, 8, 1], num_classes)
...
def SqNxt_23_1x_GELU(num_classes):
    return SqueezeNext_GELU(2.0, [6, 6, 8, 1], num_classes)
...
def SqNxt_23_1x_Smish(num_classes):
    return SqueezeNext_Smish(2.0, [6, 6, 8, 1], num_classes)
...
SmishNet = SqNxt_23_1x_Smish(100)
...
def SqNxt_23_1x_Logish(num_classes):
    return SqueezeNext_Logish(2.0, [6, 6, 8, 1], num_classes)
...
LogishNet = SqNxt_23_1x_Logish(100)
...
def SqNxt_23_1x_ReLU(num_classes):
    return SqueezeNext_ReLU(2.0, [6, 6, 8, 1], num_classes)
...
def SqNxt_23_1x_Mish(num_classes):
    return SqueezeNext_Mish(2.0, [6, 6, 8, 1], num_classes)
...
def SqNxt_23_1x_SiLU(num_classes):
    return SqueezeNext_SiLU(2.0, [6, 6, 8, 1], num_classes)
...
def train(epoch, activation):
    ...
    correct    = 0
    total      = 0
    optimizer  = optim.SGD(net.parameters(), lr=lr_schedule(lr, epoch), momentum=0.0, weight_decay=0.003)
...
torch.save(bestSishNet.state_dict(), "SqueezeNet_CIFAR100_AdamW_bestSishNet.pt")
torch.save(bestGELUNet.state_dict(), "SqueezeNet_CIFAR100_AdamW_bestGELUNet.pt")
#torch.save(bestSmishNet.state_dict(), "SqueezeNet_CIFAR100_AdamW_bestSmishNet.pt")
#torch.save(bestLogishNet.state_dict(), "SqueezeNet_CIFAR100_AdamW_bestLogishNet.pt")
torch.save(bestReLUNet.state_dict(), "SqueezeNet_CIFAR100_AdamW_bestReLUNet.pt")
#torch.save(bestMishNet.state_dict(), "SqueezeNet_CIFAR100_AdamW_bestMishNet.pt")
#torch.save(bestSiLUNet.state_dict(), "SqueezeNet_CIFAR100_AdamW_bestSiLUNet.pt")

with open('SqueezeNet_CIFAR100_Trial9.csv', 'w') as myfile:
...
print("SqueezeNet CIFAR100 trial complete")

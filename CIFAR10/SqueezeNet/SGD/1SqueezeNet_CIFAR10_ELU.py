...
class SqueezeNext_ELU(nn.Module):
    def __init__(self, width_x, blocks, num_classes):
        super(SqueezeNext_ELU, self).__init__()
        self.in_channels = 64
    ...
    def _make_layer(self, num_block, width_x, out_channels, stride):
        ...
        for _stride in strides:
            layers.append(BasicBlock_ELU(int(width_x * self.in_channels), int(width_x * out_channels), _stride))
...
def SqNxt_23_1x_ELU(num_classes):
    return SqueezeNext_ELU(1.0, [6, 6, 8, 1], num_classes)
...
def train(epoch, activation):
    ...
    correct    = 0
    total      = 0
    optimizer  = optim.SGD(net.parameters(), lr=lr_schedule(lr, epoch), momentum=0.0, weight_decay=3e-3)

...
transform_train = transforms.Compose([
    ...
    #transforms.v2.ColorJitter(brightness=0.5,hue=0.3),
    transforms.ToTensor(),
...
def ResNet18_Sish():
    return ResNet_Sish(BasicBlock_Sish, [2, 2, 2, 2])
...
class BasicBlock_ELU(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1):
        super(BasicBlock_ELU, self).__init__()
        self.conv1 = nn.Conv2d(
            in_planes, planes, kernel_size=3, stride=stride, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(planes)
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=3,
...
class ResNet_ELU(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10):
        super(ResNet_ELU, self).__init__()
        self.in_planes = 64
...
def ResNet18_ELU():
    return ResNet_ELU(BasicBlock_ELU, [2, 2, 2, 2])
...
def ResNet18_Smish():
    return ResNet_Smish(BasicBlock_Smish, [2, 2, 2, 2])
...
def ResNet18_Logish():
    return ResNet_Logish(BasicBlock_Logish, [2, 2, 2, 2])
...
def ResNet18_ReLU():
    return ResNet_ReLU(BasicBlock_ReLU, [2, 2, 2, 2])
...
def ResNet18_Mish():
    return ResNet_Mish(BasicBlock_Mish, [2, 2, 2, 2])
...
def ResNet18_SiLU():
    return ResNet_SiLU(BasicBlock_SiLU, [2, 2, 2, 2])
...
def train(epoch, activation):
    ...
    correct    = 0
    total      = 0
    optimizer  = optim.AdamW(net.parameters(), lr=lr_schedule(lr, epoch), weight_decay=7e-3)
...
with open('ResNet_NO_CIFAR10_Trial1.csv', 'w') as myfile:

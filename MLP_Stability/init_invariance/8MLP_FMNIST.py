...
def logish(input):
    return input * torch.log(1 + F.sigmoid(input))

def smish(input):
    return input * torch.tanh(torch.log(1 + F.sigmoid(input)))

telugain = init.calculate_gain("relu")
relugain = init.calculate_gain("relu")
gelugain = init.calculate_gain("relu")
silugain = init.calculate_gain("relu")
mishgain = init.calculate_gain("relu")
elugain = init.calculate_gain("relu")
logishgain = init.calculate_gain("relu")
smishgain = init.calculate_gain("relu")

print("telugain ", telugain)
print("relugain ", relugain)
print("gelugain ", gelugain)
print("silugain ", silugain)
print("mishgain ", mishgain)
print("elugain ", elugain)
print("logishgain ", logishgain)
print("smishgain ", smishgain)

def init_weights_bias_xavier_normal(m):
    class_name = m.__class__.__name__
    if class_name.find('Linear') != -1:
        init.xavier_normal_(m.weight, gain=telugain)
        init.constant_(m.bias, 0)

def init_weights_bias_xavier_uniform(m):
    class_name = m.__class__.__name__
    if class_name.find('Linear') != -1:
        init.xavier_uniform_(m.weight, gain=telugain)
        init.constant_(m.bias, 0)

def init_weights_bias_xavier_normal_relu(m):
    class_name = m.__class__.__name__
    if class_name.find('Linear') != -1:
        init.xavier_normal_(m.weight, gain=relugain)
        init.constant_(m.bias, 0)

def init_weights_bias_xavier_uniform_relu(m):
    class_name = m.__class__.__name__
    if class_name.find('Linear') != -1:
        init.xavier_uniform_(m.weight, gain=relugain)
        init.constant_(m.bias, 0)

def init_weights_bias_xavier_uniform_gelu(m):
    class_name = m.__class__.__name__
    if class_name.find('Linear') != -1:
        init.xavier_uniform_(m.weight, gain=gelugain)
        init.constant_(m.bias, 0)

def init_weights_bias_xavier_normal_gelu(m):
    class_name = m.__class__.__name__
    if class_name.find('Linear') != -1:
        init.xavier_normal_(m.weight, gain=gelugain)
        init.constant_(m.bias, 0)

def init_weights_bias_xavier_uniform_silu(m):
    class_name = m.__class__.__name__
    if class_name.find('Linear') != -1:
        init.xavier_uniform_(m.weight, gain=silugain)
        init.constant_(m.bias, 0)

def init_weights_bias_xavier_normal_silu(m):
    class_name = m.__class__.__name__
    if class_name.find('Linear') != -1:
        init.xavier_normal_(m.weight, gain=silugain)
        init.constant_(m.bias, 0)

def init_weights_bias_xavier_uniform_mish(m):
    class_name = m.__class__.__name__
    if class_name.find('Linear') != -1:
        init.xavier_uniform_(m.weight, gain=mishgain)
        init.constant_(m.bias, 0)

def init_weights_bias_xavier_normal_mish(m):
    class_name = m.__class__.__name__
    if class_name.find('Linear') != -1:
        init.xavier_normal_(m.weight, gain=mishgain)
        init.constant_(m.bias, 0)

def init_weights_bias_xavier_uniform_elu(m):
    class_name = m.__class__.__name__
    if class_name.find('Linear') != -1:
        init.xavier_uniform_(m.weight, gain=elugain)
        init.constant_(m.bias, 0)

def init_weights_bias_xavier_normal_elu(m):
    class_name = m.__class__.__name__
    if class_name.find('Linear') != -1:
        init.xavier_normal_(m.weight, gain=elugain)
        init.constant_(m.bias, 0)

def init_weights_bias_xavier_uniform_logish(m):
    class_name = m.__class__.__name__
    if class_name.find('Linear') != -1:
        init.xavier_uniform_(m.weight, gain=logishgain)
        init.constant_(m.bias, 0)

def init_weights_bias_xavier_normal_logish(m):
    class_name = m.__class__.__name__
    if class_name.find('Linear') != -1:
        init.xavier_normal_(m.weight, gain=logishgain)
        init.constant_(m.bias, 0)

def init_weights_bias_xavier_uniform_smish(m):
    class_name = m.__class__.__name__
    if class_name.find('Linear') != -1:
        init.xavier_uniform_(m.weight, gain=smishgain)
        init.constant_(m.bias, 0)

def init_weights_bias_xavier_normal_smish(m):
    class_name = m.__class__.__name__
    if class_name.find('Linear') != -1:
        init.xavier_normal_(m.weight, gain=smishgain)
        init.constant_(m.bias, 0)



def init_weights_bias_he_normal(m):
    class_name = m.__class__.__name__
    if class_name.find('Linear') != -1:
        init.kaiming_normal_(m.weight)
        init.constant_(m.bias, 0)

def init_weights_bias_he_uniform(m):
    class_name = m.__class__.__name__
    if class_name.find('Linear') != -1:
        init.kaiming_uniform_(m.weight)
        init.constant_(m.bias, 0)

def init_weights_bias_ones(m):
    class_name = m.__class__.__name__
    if class_name.find('Linear') != -1:
        init.ones_(m.weight)
        init.constant_(m.bias, 0)

#init_weights_bias = init_weights_bias_xavier_normal
...
TeLUNet.apply(init_weights_bias_xavier_normal)
if is_use_cuda:
    TeLUNet.to(device)
...
LogishNet.apply(init_weights_bias_xavier_normal)
if is_use_cuda:
    LogishNet.to(device)
...
SmishNet.apply(init_weights_bias_xavier_normal)
if is_use_cuda:
    SmishNet.to(device)
...
GELUNet.apply(init_weights_bias_xavier_normal)
if is_use_cuda:
    GELUNet.to(device)
...
ELUNet.apply(init_weights_bias_xavier_normal)
if is_use_cuda:
    ELUNet.to(device)
...
MishNet.apply(init_weights_bias_xavier_normal)
if is_use_cuda:
    MishNet.to(device)
...
SiLUNet.apply(init_weights_bias_xavier_normal)
if is_use_cuda:
    SiLUNet.to(device)
...
ReLUNet.apply(init_weights_bias_xavier_normal)
if is_use_cuda:
    ReLUNet.to(device)
...
with open('MLP_Init_Trial_XN_8.csv', 'w') as myfile:
